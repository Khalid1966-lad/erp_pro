...
import {
  ...
  Inbox, Wallet, CalendarDays, Printer, AlertTriangle, BarChart3, Scale, CreditCard, Truck,
} from 'lucide-react'
...
// ═══════════════════════════════════════════════════════════════
//  FINANCIAL STATEMENT TAB (Relevé de Compte Fournisseur)
// ═══════════════════════════════════════════════════════════════
...
interface SupplierStatementData {
  supplier: { id: string; name: string; code: string; siret: string | null; phone: string | null; email: string | null; address: string | null; city: string | null; postalCode: string | null }
  from: string | null
  to: string | null
...
function SupplierFinancialStatementTab({ supplierId }: { supplierId: string }) {
  ...
  const fetchStatement = useCallback(async () => {
    try {
      ...
      const res = await api.get<SupplierStatementData>(`/suppliers/${supplierId}/statement?${params}`)
      setData(res)
    } catch (err: any) {
  ...
  const handlePrint = async () => {
    ...
    const infoGrid = [
      { label: 'Fournisseur', value: data.supplier.name || '—' },
      { label: 'Code', value: data.supplier.code || '—' },
      { label: 'Téléphone', value: data.supplier.phone || '—' },
    ...
    for (const tx of data.transactions) {
      const typeLabel = tx.type === 'invoice' ? 'Facture' : tx.type === 'credit_note' ? 'Avoir' : tx.type === 'rejet_effet' ? 'Rejet effet' : 'Paiement'
      rows.push([
    ...
    // Build récapitulatif financier sub-section for print
    let subSections = ''
    if (data.summary) {
        ...
        <table>
          ...
          <tbody>
            <tr>
              ...
              <td class="text-right" style="${s.uninvoicedReceptions > 0 ? 'color:#ea580c;font-weight:600;' : ''}">${fmtM(s.uninvoicedReceptions)}</td>
  ...
  const summaryRows = data?.summary ? [
    ...
    { label: 'Réception', sublabel: 'Non facturées', value: data.summary.uninvoicedReceptions, icon: Truck, color: data.summary.uninvoicedReceptions > 0 ? 'text-orange-600' : '' },
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="info" className="space-y-4">
        ...
        <TabsContent value="financial">
          <SupplierFinancialStatementTab supplierId={supplier.id} />
        </TabsContent>
